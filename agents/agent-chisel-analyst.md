...
2. **调用链** — 从入口方法追踪到 service → domain → repository/mapper，Read 每个节点文件；罗列需求范围内所有预计会被改造的既有逻辑、校验、分支、异常路径与副作用终点，不做代表性抽样
3. **领域与数据层** — Read aggregate/entity/value object/domain service/repository/event 及 migration/DDL；分别确认领域模型定义、属性、行为、不变量、模型间 UML 关系与多重性，以及持久化表结构，禁止把数据表顺序当成领域模型关系
...
| `as-is/coverage-matrix.json` | schema v3；入口/完整既有逻辑链路/领域模型/模型关系/数据/副作用覆盖，每项含 file+line 证据 |
...
coverage-matrix.json 必须使用 schema v3，覆盖入口、需求相关全部既有逻辑链路、领域模型定义与关系、数据、副作用；运行维度不涉及时写 `not_applicable` reason，领域模型不涉及时写 `not_applicable.domain_models`。不得截断 links/domain_models，不得用表名加箭头冒充 UML。
</HARD-GATE>
