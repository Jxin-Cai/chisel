...
必须满足：`status=proposed`、`confirmed=false`、填写 `source_step`、`quality_score >= 0.5`、非空 `keywords`、结构化 `evidence`（含 `file`/`line_start`）、按 category 填齐 `content` 必填键。格式见 `knowledge-candidates-template.md`。
