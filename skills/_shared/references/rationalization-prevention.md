...
| "as-is 已经确认了，AI 输入版可以跳过直接做方案" | AI 输入版是 Planner 的结构化输入，gate 会检查 6 个文件是否存在。clarifications.md 也必须存在。 |
| "知识候选很明显，不用等用户确认直接合入 wiki" | wiki 是长期知识，合入前必须用户逐条确认。自动合入会导致错误知识污染后续迭代。 |
