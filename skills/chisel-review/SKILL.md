...
```bash
# 优先从 worktree-decision.json 读取创建分支时的 base commit
node -e "const d=JSON.parse(require('fs').readFileSync('{IDEA_DIR}/worktree-decision.json','utf8')); console.log(d.base_commit || '')" 2>/dev/null || echo ""
```
