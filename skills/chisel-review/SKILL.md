...
分两批启动，避免 7 个 opus agent 同时竞争导致 stall：

6a. **Batch 1**——在一条消息中发起 4 个 Agent 调用：
   ```
...
7. 等待所有 7 个 Agent 返回后，逐个运行 `node ${CLAUDE_PLUGIN_ROOT}/scripts/cr-parse.mjs {IDEA_DIR} --dim <dimension>` 解析结果
8. 聚合判定：
...
D2-D8 分两批（4+3）发起 Agent 调用，每批返回后再发下一批，全部返回后聚合结果。
聚合后一次性收集所有问题避免反复返修。
...
| "只有一个 task，不需要完整流程" | 单 task 也走完整 8 维度 |
| "diff 不够看，先读全文件再审" | diff 是主材料，全文件只是补充——先从 diff 出发 |
| "6 个维度同时跑更快" | 并发过高会导致全部 stall，分两批（4+3）更可靠 |
