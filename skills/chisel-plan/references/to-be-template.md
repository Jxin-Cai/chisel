...
每个行为变更必须列出明确的单测 RED → GREEN 证据计划：

1. 先写能因目标行为缺失而失败的具体测试（写出测试文件、测试名、输入和断言）。
2. 运行该测试并记录预期失败原因；测试立即通过或因语法/环境报错都不算有效 RED。
3. 只实现使该测试通过的最小代码。
4. 重跑该测试和相关完整测试集，记录预期 PASS。
5. 明确覆盖率命令、覆盖率产物路径和 lines/branches/functions/statements 阈值。

禁止使用“补充相关测试”“覆盖边界情况”等无测试名、无命令、无断言的占位描述。
