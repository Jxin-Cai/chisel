...
整理本轮用户澄清中发现的长期知识候选，经用户逐条决策后合入 wiki 或标记终态。阶段完成 = 所有候选进入终态且通过 `knowledge-extracted` gate。

知识候选的核心定位：**代码之外的人类上下文**——用户在对话中澄清的历史背景、业务术语映射、约束决策等代码本身无法推导的信息。代码扫描能得到的信号（坏味道、指标异常）不应进入候选池。
