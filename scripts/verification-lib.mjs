...
export function validateVerificationResult(ideaDir, projectRoot = '.') {
  ...
  for (const repo of repositories) {
    if (repo.status && repo.status !== 'pass') return `verification status must be pass for ${repo.project_root || projectRoot}`;
    if (!Array.isArray(repo.checks) || repo.checks.length === 0) return `verification checks must be non-empty for ${repo.project_root || projectRoot}`;
    const failed = repo.checks.filter(check => check.status !== 'pass' || check.exit_code !== 0);
    if (failed.length > 0) return `verification contains failed checks for ${repo.project_root || projectRoot}: ${failed.map(check => check.id).join(', ')}`;
    const current = workspaceIdentity(repo.project_root || projectRoot);
    if (current.error) return current.error;
    if (repo.git_head !== current.head) return `verification is stale: git HEAD changed for ${repo.project_root || projectRoot}`;
    if (repo.workspace_fingerprint !== current.fingerprint) return `verification is stale: working tree changed for ${repo.project_root || projectRoot}`;
  }
  return '';
}
