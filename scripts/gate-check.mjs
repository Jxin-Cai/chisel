...
function validateTaskIntegrity(ideaDir) {
  ...
  for (const [taskId, task] of entries) {
    ...
    if (!Object.hasOwn(fm, 'starting_points') && !Object.hasOwn(fm, 'expected_files')) {
      return `${taskId} frontmatter missing starting_points`;
    }
    ...
    if (!arraysEqual(task.expected_files || [], startingPoints)) {
      return `${taskId} starting_points mismatch between state and task file`;
    }
  }
...
function validateScopeProof(text, mode, { behaviorInvariants = [], requireAllInvariantPass = false } = {}) {
  ...
  if (!/schema_version[：:]\s*[234]\b/.test(text)) return 'missing scope proof schema_version: 2, 3, or 4';
  if (!/violations_count[：:]\s*\d+\b/.test(text)) return 'missing numeric scope proof violations_count';
  if (/schema_version[：:]\s*[34]\b/.test(text) && !text.includes('Invariant Proofs')) return 'schema_version 3/4 review proof must include Invariant Proofs';
...
function validateTaskReport(reportPath, behaviorInvariants = [], traceRefs = [], { ideaDir = '', taskId = '', taskText = '', taskFm = {} } = {}) {
  ...
  const reportFm = readFrontmatter(text);
  if (Number(reportFm.report_schema_version || 0) >= 4) {
    if (reportFm.generated_by !== 'task-metrics.mjs') return 'automated task inventory must be generated by task-metrics.mjs';
    let run;
    try { run = taskId ? readTaskRun(ideaDir, taskId) : null; }
    catch (error) { return error.message; }
    const provenance = run?.attempts?.[run.attempts.length - 1];
    if (!provenance?.finished_at) return 'task provenance attempt is not finished';
    const reported = Array.isArray(reportFm.changed_files) ? [...reportFm.changed_files].sort() : [];
    const actual = [...(provenance.changed_files || [])].sort();
    if (JSON.stringify(reported) !== JSON.stringify(actual)) {
      return `automated changed_files do not match task provenance (expected: ${actual.join(', ') || '<none>'})`;
    }
    const scope = checkScope(ideaDir, taskId, '.');
    if (!scope.pass) return `explicit forbidden boundary violated: ${scope.violations.map(item => `${item.file}:${item.type}`).join(', ')}`;
    return '';
  }
  ...
  if (isNewFileReportContract(ideaDir, taskText, taskFm, reportFm)) {
    let run = null;
    try { run = taskId ? readTaskRun(ideaDir, taskId) : null; }
    catch (error) { return error.message; }
...
function validateQuickDevScopeContract(ideaDir) {
  ...
  const taskExpected = normalizeTraceRefs(taskFm.starting_points || taskFm.expected_files);
  if (JSON.stringify(taskExpected) !== JSON.stringify(normalizeTraceRefs(doc.expected_files))) return 'quick-dev-scope.json expected_files do not match task starting_points';
