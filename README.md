...
5. **策略设计**（`plan:strategy`）：planner agent 产出 `to-be/implementation-plan.md`，聚焦实现方向、设计决策、允许/禁止修改范围和保障策略。
...
8. **用户确认拆分**（`plan:decompose-confirm`）：确认 task 拆分和依赖关系，写入 `confirmations/to-be.json`。
...
11. **架构师 CR**（`review:cr`）：reviewer agent 只读审查，逐项检查验收标准和行为不变式。
...
planner 基于已确认策略产出 task 拆分后，chisel 展示每个 task 的目标、依赖和风险。用户确认后写入 `confirmations/to-be.json`。
